// src/pages/FollowersPage.tsx (updated)
...
export default function FollowersPage() {
  ...
  const currentFollowers = allFollowers.slice(startIndex, startIndex + followersPerPage);

  const toggleFollow = (userId: number, currentlyFollowing: boolean) => {
    console.log(currentlyFollowing ? 'Unfollow' : 'Follow', userId);
  };
    ...
    <div className="mx-auto p-4 w-full max-w-150">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="text-right">
          <p className="font-semibold text-foreground text-sm">12 following</p>
          <p className="text-muted-foreground text-xs">You follow 12 accounts</p>
        </div>
      </div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <header className="flex items-baseline gap-2 mb-1">
                <span className="font-semibold text-sm truncate">{follower.username}</span>
                <span className="text-muted-foreground text-xs">·</span>
                <span className="text-muted-foreground text-xs truncate">{follower.handle}</span>
              </header>
            ...
            <Button
              ...
              onClick={() => toggleFollow(follower.id, follower.following)}
            >
